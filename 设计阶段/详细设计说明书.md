...
| 版本号 | 版本描述     | 责任人 | 日期      | 备注 |
| ------ | ------------ | ------ | --------- | ---- |
| V1.0   | 初始编制     | 张一鸣 | 2020-6-16  |      |

## 1.影片自动裁切模块

### 1.1 文件关系

![文件关系视图](https://www.z4a.net/images/2020/06/26/pyfilm-spliter.png)
...
对指定位置的字幕进行分析，并寻找其共同目录下的电影切割为音频文件，同时将分析结果上传到数据库。
...
**注：程序启动需要接受一个配置文件（setting.xml）其标准如下：**
...
<setting>
    ...
    <path>
        <caption_path>这里填写字幕与对于视频的目录</caption_path>
...
class Tick:
    ...
    #将字幕文件的精确到小数点后三位的时间数据转换为精确到小数点后两位，便于影片切割模块裁切视频
...
![文件关系视图](https://www.z4a.net/images/2020/06/26/word_api.png)
...
#将用户发送的的原始json字符串反序列化为Receive对象
